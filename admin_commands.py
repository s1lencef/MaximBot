        ...
        if args[1] == '+':
            ...
            if(len(args) == 2):
                buttons = [InlineKeyboardButton("Изменить сумму", callback_data=f"loyalty_level#{args[1]}#edit_sum"),
                           InlineKeyboardButton("Изменить процент", callback_data=f"loyalty_level#{args[1]}#edit_coeff")]
                menu = build_menu(buttons, 2,footer_buttons = [InlineKeyboardButton("Вернуться", callback_data="loyalty_level#back")] )
                reply_markup = InlineKeyboardMarkup(menu)
...
async def sum_level(update,context):
    ...
    await update.message.reply_text(text="Введите процент ",reply_markup=cancel_reply_markup,parse_mode=ParseMode.HTML)
    return 1
...
async def change_user(update,context):
    ...
    try:
        ...
        await update.message.reply_text("Пользователь с таким id не найден!\nВведите id:", parse_mode=ParseMode.HTML,reply_markup=InlineKeyboardMarkup(build_menu(
...
async def delete_user(update,context):
    ...
    try:
        ...
        print(e)
        await update.message.reply_text("Пользователь с таким id не найден!\nВведите id:",reply_markup=InlineKeyboardMarkup(build_menu(
        [InlineKeyboardButton('Отмена', callback_data='cancel')],1)))
        return 4
    context.user_data['user_id'] = id
...
async def add_points(update,context):
    ...
    user.save()
    await context.bot.send_message(chat_id=user.id, text=user_text, parse_mode=ParseMode.HTML)
    await update.message.reply_text(f"Теперь у пользователя <b>{user.loyalty_points}</b> баллов", parse_mode=ParseMode.HTML)
    return ConversationHandler.END
